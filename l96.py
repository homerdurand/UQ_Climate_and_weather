...
def plot_l96_polar_animation(X_hist, Y_hist, save_path='l96_polar_animation.mp4', fps=15):
    K = X_hist.shape[1] 
    J = Y_hist.shape[1] 
    ...
    theta_Y = np.append(theta_Y, theta_Y[0])  
    ...
    def update(frame):
        X_values = np.append(X_hist[frame], X_hist[frame][0])  
        Y_values = np.append(Y_hist[frame], Y_hist[frame][0]) 
        
        line_X.set_data(theta, X_values)
        line_Y.set_data(theta_Y, Y_values)
        return line_X, line_Y
    
    ani = animation.FuncAnimation(fig, update, frames=len(X_hist), init_func=init, blit=True)
...
class L96TwoLevel(object):
    ...
    def step(self, add_B=True, B=None):
        ...
        if self.parameterization is None:
            ...
            B = self.parameterization(self.X[None, :])[0, :] if B is None else B
            self.X += 1 / 6 * (k1_X + 2 * k2_X + 2 * k3_X + k4_X)
